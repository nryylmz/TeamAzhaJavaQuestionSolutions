...
public class SortAscend {
    public static int[] sortAscending(int[] arr) {/*return method with int array return type.Takes an array of integers*/
        ...
        for (int i = 0; i < arr.length - 1; i++) { /*nested for-loop. The outer loop is initialized
            ...
            for (int j = 0; j < arr.length - i - 1; j++) {/*inner loop to iterate over the unsorted portion
                ...
                if (arr[j] > arr[j + 1]) {/*checked if the current element is greater than the next element i.e., wrong order.*/
            ...
            return arr;/*sorted array is returned once the iteration ends and loop is complete*/
        }
